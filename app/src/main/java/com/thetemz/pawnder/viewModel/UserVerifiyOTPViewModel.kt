...
import com.thetemz.pawnder.resporities.VerifiyOTPRespository
...
class UserVerifiyOTPViewModel(application: Application) : AndroidViewModel(application) {

    private val UserVerifiyOTP = VerifiyOTPRespository(APIFactory.makeServiceAPi())
    ...
    private val liveDataVerifiyOTP = MutableLiveData<SucessFullyResModel>()
    ...
    fun UserSendOTP(
        ...
        scope.launch {
            ...
            var response = UserVerifiyOTP.getSendOTP(phone,otp)


            response?.let {
                if (it.status == true) {
                    //   liveDataLoginWithPassword.postValue(it.result)
                    liveDataVerifiyOTP.postValue(it)
